...
FEATURE_METADATA_VERSION = os.getenv("BOOK_CONTENT_FEATURE_METADATA_VERSION", "book_feature_map_v1")
...
def _normalize_book_id(raw_id: Any, fallback: str) -> str:
    normalized = str(raw_id or fallback).strip()
...
def _normalize_book_record(book: Dict[str, Any], fallback_id: str, source: str) -> Dict[str, Any]:
    book_id = _normalize_book_id(book.get("book_id") or book.get("id"), fallback_id)
    title = str(book.get("title") or "").strip() or book_id
    ...
    normalized = {
        **book,
        ...
        "genres": _normalize_genres(book.get("genres") or []),
...
def _build_book_records(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for index, book in enumerate(source_books):
        if not isinstance(book, dict):
            continue
        normalized = _normalize_book_record(book, fallback_id=f"book_{index}", source="books_payload")
        existing = by_id.get(normalized["book_id"])
        if existing:
            merged = {**existing, **normalized}
            merged["candidate_source"] = "books_payload"
            by_id[normalized["book_id"]] = merged
            continue
        by_id[normalized["book_id"]] = normalized

    source_candidate_ids = payload.get("candidate_ids") or []
    for candidate_id in source_candidate_ids:
        cid = _normalize_book_id(candidate_id, fallback="unknown_candidate")
        if not cid or cid == "unknown_candidate":
            continue
        if cid in by_id:
            continue
        by_id[cid] = {
            ...
            "description": "",
            "genres": [],
            "candidate_source": "candidate_ids",
        }
        ...
        books = [
            {
                ...
                "genres": [],
                "candidate_source": "ingest_batch_fallback",
            }
        ]
    return books
        ...
        vectors.append(
            {
                ...
                "candidate_source": book.get("candidate_source", "unknown"),
            }
        )
...
async def _analyze_content(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    outputs = {
        ...
        "features_metadata": {
            ...
            "candidate_sources": sorted({str(book.get("candidate_source") or "unknown") for book in books}),
        },
