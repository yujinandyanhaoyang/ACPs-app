...
| 对比项 | 迁移前 | 迁移后 |
|--------|--------|--------|
| **嵌入后端** | sentence-transformers | DashScope qwen3-vl-embedding（多模态 API） |
| **存储占用** | 7.3 GB | 0 MB |
| **向量维度** | 384 维 | 1536 维 |
| **语义质量** | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| **中文支持** | 一般 | 优秀 |
...
# 查看日志，确认嵌入后端
# 应显示：backend=dashscope-multimodal, model=qwen3-vl-embedding, vector_dim=1536
```
...
| 模型 | 价格 | 最大输入 |
|------|------|----------|
| qwen3-vl-embedding | **订阅制**（已付费） | 支持多模态 |
| text-embedding-v3 | ¥0.0007 / 1000 tokens | 8192 tokens |

> ⚠️ **注意**：本项目仅使用 `qwen3-vl-embedding`（订阅制，已付费），避免使用按量计费的 `text-embedding-v3` 产生额外费用。
...
print(f"Backend: {meta['backend']}")  # dashscope-multimodal
