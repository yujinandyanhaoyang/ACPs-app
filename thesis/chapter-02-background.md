...
根据 Agent 的角色和能力进行任务分配。本研究的 ACPs-app 推荐系统采用 Leader-Partner 架构：
- **ReadingConcierge (Leader)**: 任务编排、结果整合、会话管理
- **ReaderProfile (Partner)**: 用户偏好建模、情感分析
- **BookContent (Partner)**: 图书特征提取、嵌入生成、知识图谱查询
- **RecRanking (Partner)**: 多因子融合排序、解释生成
...
典型的推荐任务执行流程（Leader-Partner 架构）：
1. ReadingConcierge → ReaderProfile: StartTask（用户画像分析）
2. ReaderProfile → ReadingConcierge: Completed（返回偏好向量）
3. ReadingConcierge → BookContent: StartTask（图书内容分析）
4. BookContent → ReadingConcierge: Completed（返回内容向量）
5. ReadingConcierge → RecRanking: StartTask（多因子排序）
6. RecRanking → ReadingConcierge: Completed（返回排序结果）
...
embedding = model.encode('这本书很好看', convert_to_numpy=True, normalize_embeddings=True)
```
