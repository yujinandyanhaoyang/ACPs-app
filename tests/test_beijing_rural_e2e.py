"""
beijing_rural 的端到端 HTTP 集成测试（需要真实服务进程）。

运行示例：
    export END_TO_END=1
    export BEIJING_RURAL_BASE_URL=http://localhost:8012
    export BEIJING_RURAL_AIP_ENDPOINT=/acps-aip-v1/rpc
    venv/bin/python -m pytest -q tests/test_beijing_rural_e2e.py
"""
...
BASE_URL = os.getenv("BEIJING_RURAL_BASE_URL", "http://localhost:8012").rstrip("/")
ENDPOINT = os.getenv("BEIJING_RURAL_AIP_ENDPOINT", "/acps-aip-v1/rpc")
...
def test_http_live_start_then_background():
    ...
    body = _post(_build_rpc_payload("计划在北京郊区进行1日自然深度游。", task_id))
...
def test_http_live_reject_out_of_scope():
    """验证：超出郊区范围（城区）的请求，返回 Rejected 或 AwaitingInput（提示澄清）。"""
    task_id = f"task-{uuid.uuid4()}"
    body = _post(_build_rpc_payload("请规划故宫一日游（城区）。", task_id))
...
def test_http_live_continue_to_awaiting_completion():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("需要北京郊区一日游建议", task_id))
    st, _ = _wait_for_state(task_id, {"awaiting-input", "working"})
    assert st in {"awaiting-input", "working"}
    cont = _post(
        _build_rpc_payload(
            "主题=自然摄影；必看=慕田峪长城；预算=中等。", task_id, command="continue"
        )
    )
    ...
    if st2 == "awaiting-input":
        cont2 = _post(
            _build_rpc_payload(
                "主题=自然摄影；必看=慕田峪、怀柔水库；天数=1；预算=中等；路况偏好=徒步+摆渡。",
                task_id,
                command="continue",
            )
        )
...
def test_http_live_complete_flow():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("北京郊区1日游，自然主题。", task_id))
    st, _ = _wait_for_state(
    ...
    if st == "awaiting-input":
        _post(
            _build_rpc_payload(
                "主题=自然摄影；必看=慕田峪长城；预算=中等。",
                task_id,
                command="continue",
            )
        )
        st, _ = _wait_for_state(task_id, {"awaiting-completion", "awaiting-input"})
        if st == "awaiting-input":
            _post(
                _build_rpc_payload(
                    "主题=自然摄影；必看=慕田峪、怀柔水库；天数=1；预算=中等；路况偏好=徒步+摆渡。",
            ...
            st, _ = _wait_for_state(task_id, {"awaiting-completion"}, timeout_s=6.0)
...
def test_http_live_cancel_flow():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("北京郊区1日游。", task_id))
    st, _ = _wait_for_state(
...
def test_http_live_awaiting_input_timeout_to_canceled():
    ...
    _post(_build_rpc_payload("需要北京郊区一日游建议", task_id, command_params=params))
...
def test_http_live_products_size_limit_failed():
    ...
    if st == "awaiting-input":
        ...
        st, _ = _wait_for_state(task_id, {"failed", "awaiting-input"}, timeout_s=6.0)
        if st == "awaiting-input":
            _post(
                _build_rpc_payload(
                    "主题=自然摄影；必看=慕田峪、怀柔水库；天数=1；预算=中等；路况偏好=徒步+摆渡。",
...
def test_http_live_continue_ignored_when_terminal_rejected():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("请规划故宫一日游（城区）。", task_id))
    st, _ = _wait_for_state(task_id, {"rejected", "awaiting-input"})
    if st == "awaiting-input":
        pytest.skip("分析未直接拒绝而是引导澄清，跳过。")
    assert st == "rejected"
    res = _post(_build_rpc_payload("追加信息：改为郊区", task_id, command="continue"))
