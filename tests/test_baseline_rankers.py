from services.baseline_rankers import traditional_hybrid_rank, multi_agent_proxy_rank, llm_only_rank
...
def test_llm_only_rank_uses_llm_selected_ids(monkeypatch):
    case_payload = {
        "query": "recommend space opera",
        "history": [{"title": "Dune", "genres": ["science_fiction"], "rating": 5}],
    }
    ...
    rows = llm_only_rank(case_payload, top_k=2)
    assert len(rows) == 2
    assert [row["book_id"] for row in rows] == ["b2", "b1"]
