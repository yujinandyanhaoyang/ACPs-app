"""
beijing_urban 的端到端 HTTP 集成测试（需要真实服务进程）。

运行示例：
    export END_TO_END=1
    export BEIJING_URBAN_BASE_URL=https://localhost:8011
    export BEIJING_URBAN_AIP_ENDPOINT=/acps-aip-v1/rpc
    venv/bin/python -m pytest -q tests/test_beijing_urban_e2e.py
"""
...
BASE_URL = os.getenv("BEIJING_URBAN_BASE_URL", "https://localhost:8011").rstrip("/")
ENDPOINT = os.getenv("BEIJING_URBAN_AIP_ENDPOINT", "/acps-aip-v1/rpc")
...
def _post(payload: dict, timeout=30):
    try:
        # 禁用SSL证书验证（用于自签名证书的测试环境）
        r = requests.post(RPC_URL, json=payload, timeout=timeout, verify=False)
...
def test_http_live_start_then_background():
    ...
    body = _post(_build_rpc_payload("计划在北京城区进行1日文化深度游。", task_id))
...
def test_http_live_reject_out_of_scope():
    """验证：超出城区范围的请求，返回 Rejected 或 AwaitingInput（提示澄清）。"""
    task_id = f"task-{uuid.uuid4()}"
    body = _post(_build_rpc_payload("请规划怀柔/密云山区两日游（郊区）。", task_id))
...
def test_http_live_continue_to_awaiting_completion():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("需要北京城区一日游建议", task_id))
    st, _ = _wait_for_state(task_id, {"awaiting-input", "working"})
    assert st in {"awaiting-input", "working"}
    cont = _post(
        _build_rpc_payload(
            "主题=文化深度；必看=故宫；预算=中等。", task_id, command="continue"
        )
    )
    ...
    if st2 == "awaiting-input":
        ...
        cont2 = _post(
            _build_rpc_payload(
                "主题=文化深度；必看=故宫、天安门；天数=1；预算=中等；体力偏好=轻体力。",
                task_id,
                command="continue",
            )
        )
...
def test_http_live_cancel_on_completed_is_idempotent():
    ...
    st_ac, _ = _wait_for_state(
        task_id, {"awaiting-completion", "awaiting-input", "rejected"}, timeout_s=6.0
    )
    if st_ac == "rejected":
        pytest.skip("门卫拒绝了请求，跳过该用例。")
    if st_ac == "awaiting-input":
        _post(
            _build_rpc_payload(
                "主题=文化深度；必看=故宫；预算=中等。", task_id, command="continue"
            )
...
def test_http_live_complete_flow():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("北京城区1日游，文化主题。", task_id))
    st, _ = _wait_for_state(
...
def test_http_live_cancel_flow():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("北京城区1日游。", task_id))
    st, _ = _wait_for_state(
...
def test_http_live_awaiting_input_timeout_to_canceled():
    ...
    _post(_build_rpc_payload("需要北京城区一日游建议", task_id, command_params=params))
...
def test_http_live_continue_ignored_when_terminal_rejected():
    ...
    task_id = f"task-{uuid.uuid4()}"
    _post(_build_rpc_payload("请规划怀柔/密云山区两日游（郊区）。", task_id))
    st, _ = _wait_for_state(task_id, {"rejected", "awaiting-input"})
    if st == "awaiting-input":
        pytest.skip("分析未直接拒绝而是引导澄清，跳过。")
    assert st == "rejected"
    res = _post(_build_rpc_payload("追加信息：改为城区", task_id, command="continue"))
